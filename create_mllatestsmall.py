...
parser.add_argument('--dataset_name', type=str, default='ml-latest-small', help='dataset name', choices=['ml-latest-small'])
parser.add_argument('--out_data_dir', type=str, default='/path/to/dir', help='output data directory')
parser.add_argument('--rating_file', type=str, default='/path/to/file', help='rating file')
args = parser.parse_args()
print(args)

os.makedirs(args.out_data_dir, exist_ok=True)
raw_data_orig = pd.read_csv(args.rating_file)
raw_data_orig = raw_data_orig[raw_data_orig['rating'] > 2.5].reset_index().drop('index', axis=1)
raw_data_orig = raw_data_orig.drop('rating', axis=1)
raw_data_orig = raw_data_orig.drop('timestamp', axis=1)
...
raw_data_full = raw_data_full.drop(labels=['userId', 'movieId'], axis=1).rename(columns={"new_userId": "userId", "new_movieId": "movieId"})
...
def filter_triplets(tp, min_uc=5, min_sc=0):
    ...
    if min_uc > 0:
        ...
        tp = tp[tp['userId'].isin(usercount['userId'][usercount['size'] >= min_uc])]
...
raw_data, user_activity, item_popularity = filter_triplets(raw_data_full, min_uc=0, min_sc=0)
